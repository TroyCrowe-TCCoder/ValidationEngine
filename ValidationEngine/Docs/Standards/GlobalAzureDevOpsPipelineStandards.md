...
When a repository is initialized or rolled over to the current pipeline standard, copy the matching `Validate.ps1` template into the repository's root-level `Scripts/` folder and reference it from the repository README as the local validation entry point. `Scripts/` is the single mandated location for this script across all repository types — it must not be placed in `Tools/`, `scripts/`, or any other folder, regardless of prior repository convention. If a copy of `Validate.ps1` (or any equivalent local validation entry point) exists anywhere else in the repository, it must be deleted as part of the same work slice that adds or updates `Scripts/Validate.ps1`, and any reference to the old location (README, hooks, onboarding docs) must be updated to point at `Scripts/Validate.ps1`. The copied script becomes a repository-local live asset per Section 3.5 and is not template clutter once copied.
